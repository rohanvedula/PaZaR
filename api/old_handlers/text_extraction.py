...
def extract_text(url):
    ...
    for sequence_number in range(total_boxes):
        if details['text'][sequence_number].isalnum()==False:
            ...
            if(area<0.0017):
                ...
                croplist = crop.tolist()
                cropstring = "?".join([ "~".join("#".join(str(number) for number in numberlist) for numberlist in a) for a in croplist ])
                #print(crop.shape)
                #print(croplist)
                #processed_img, result = symbol_classifier.identify(crop, 5)
                response = requests.post("https://pazarsymbol.herokuapp.com/upload", data={"image_array" : cropstring})
                ...
                if result[0][0] > 0.5:
                    array_seq.append(sequence_number)
                    latex_array.append(result[0][1])
    ...
    for i, word in enumerate(details['text']):
        ...
        if i in array_seq:
            code = latex_array[array_seq.index(i)]
            if ("latex2e-OT1-_" in code):
                code = code.replace("latex2e-OT1-_", '\\')
            elif ("amssymb-OT1-_" in code):
                code = code.replace("amssymp-OT1-_", '\\')
